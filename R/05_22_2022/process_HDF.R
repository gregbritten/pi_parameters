...
I = PAR = MLD = KD = CHL_GIOP = CHL_GSM = BBP_GIOP = BBP_GSM = VGPM = CBPM = EP_VGPM = CAFE = CARBON_GIOP = CARBON_GSM <- array(NA,dim=c(Nlon,Nlat,Nfiles))
...
for(i in 1:Nfiles){
  ...
  PAR[,,i]      <- resize_bilinear(par_tmp,      xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  MLD[,,i]      <- resize_bilinear(mld_tmp,      xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  KD[,,i]       <- resize_bilinear(kd_tmp,       xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  CHL_GIOP[,,i] <- resize_bilinear(chl_giop_tmp, xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  CHL_GSM[,,i]  <- resize_bilinear(chl_gsm_tmp,  xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  BBP_GIOP[,,i] <- resize_bilinear(bbp_giop_tmp, xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  BBP_GSM[,,i]  <- resize_bilinear(bbp_gsm_tmp,  xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  VGPM[,,i]     <- resize_bilinear(vgpm_tmp,     xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  CBPM[,,i]     <- resize_bilinear(cbpm_tmp,     xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  EP_VGPM[,,i]  <- resize_bilinear(ep_vgpm_tmp,  xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
  CAFE[,,i]     <- resize_bilinear(cafe_tmp,     xin=Nlon0, xout=Nlon, yin=Nlat0, yout=Nlat)
...
save(file='processed_data/CARBON_GSM.rds',  CARBON_GSM)	
save(file='processed_data/CARBON_GIOP.rds', CARBON_GIOP)	
