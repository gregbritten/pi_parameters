...
plot_rf_rsq(fits_PBmax_chl_F_365, parm='PBmax',ylim_rmse=c(0.5,1.5),ylim_r2=c(0,1),text=expression(italic('P'['max']^{'B'})),outer=TRUE,
            ntime=60)
plot(-999,bty='n',xaxt='n',yaxt='n',xlim=c(0,1),ylim=c(0,1))
legend('topleft',legend=region_long[1:5], lty=1, col=cols,bty='n',cex=1)
plot_rf_rsq(fits_alpha_chl_F_365,parm='alpha',ylim_rmse=c(0.005,0.025),ylim_r2=c(0,1),text=expression(italic(alpha^{'B'})),
            ntime=60)
dev.off()
