f_plot_rf_rsq <- function(fits,parm,max=TRUE,ylim_rmse,ylim_r2,legend=FALSE,text,outer=FALSE,ntime){
  ...
  for(j in 1:5){
    ...
    xx <- unlist(lapply(1:ntime,function(i){
      ...
      if(parm=='Ek')    imp <- cor(fits[[j]][[i]][[1]]$predicted,fits[[j]][[i]][[2]]$Ek)^2
      if(parm=='alpha') imp <- cor(fits[[j]][[i]][[1]]$predicted,fits[[j]][[i]][[2]]$alpha)^2
      if(parm=='PBmax') imp <- cor(fits[[j]][[i]][[1]]$predicted,fits[[j]][[i]][[2]]$PBmax)^2
      imp
    }))
  ...
  #     if(parm=='Ek')    imp <- sqrt(mean((fits[[j]][[i]][[1]]$predicted-fits[[j]][[i]][[2]]$Ek)^2))
  #     if(parm=='alpha') imp <- sqrt(mean((fits[[j]][[i]][[1]]$predicted-fits[[j]][[i]][[2]]$alpha)^2))
  #     if(parm=='PBmax') imp <- sqrt(mean((fits[[j]][[i]][[1]]$predicted-fits[[j]][[i]][[2]]$PBmax)^2))
  #     imp
  #   }))
  #   lines(-(ntime-1):0,xx[ntime:1],col=cols[j])
